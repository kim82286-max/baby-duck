...
const STREAMS = {
  cheonggyecheon: {
    ...
    landmarks: [
      ...
      { id: 2, name: '광통교(종로2가)', icon: '🌉' },
      { id: 3, name: '장통교(종로3가)', icon: '🌉' },
      { id: 4, name: '세운상가 앞', icon: '🏢' },
      { id: 5, name: '오간수교(종로5가)', icon: '🌉' },
      { id: 6, name: '관수교(종로4가)', icon: '🌉' },
      { id: 7, name: '동대문 부근', icon: '🏯' },
      { id: 8, name: '황학교(신당역)', icon: '🌉' },
      { id: 9, name: '비우당교(동묘앞)', icon: '🌉' },
  ...
  seongbukcheon: {
    ...
    emoji: '🌿',
    landmarks: [
      { id: 101, name: '희망다리(한성대입구)', icon: '🌉' },
      { id: 102, name: '봄빛다리(성북구청)', icon: '🌉' },
      { id: 103, name: '한빛다리', icon: '🌉' },
      { id: 104, name: '어울다리', icon: '🌉' },
      { id: 105, name: '바람마당(돈암제일시장)', icon: '🌿' },
      { id: 106, name: '보문2교', icon: '🌉' },
      { id: 107, name: '보문3교', icon: '🌉' },
      { id: 108, name: '보문4교(영암교회)', icon: '🌉' },
      { id: 109, name: '안암1교(삼익아파트)', icon: '🌉' },
      { id: 110, name: '보문1교', icon: '🌉' },
      { id: 111, name: '안암2교(대광초교사거리)', icon: '🌉' },
    ],
  },
}

// 모든 랜드마크를 하나로 합침 (카드 표시용)
const ALL_LANDMARKS = [...STREAMS.cheonggyecheon.landmarks, ...STREAMS.seongbukcheon.landmarks]
...
function StreamMap({ sightings, onSelect, selected, landmarks, streamName }) {
  const latest = {}
  sightings.forEach((s) => {
  ...
  return (
    <div style={{ background: 'linear-gradient(135deg,#e8f4e8,#d4ecf7 50%,#e8f0e4)', borderRadius: 20, padding: '24px 16px', position: 'relative', overflow: 'hidden', border: '1px solid rgba(76,145,115,.15)', marginBottom: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16, position: 'relative' }}>
        ...
        <span style={{ fontFamily: "'Noto Sans KR',sans-serif", fontSize: 13, fontWeight: 700, color: '#2d6a4f', letterSpacing: '0.5px' }}>{streamName} 실시간 지도</span>
...
function ReportForm({ selectedLandmark, onSubmit, onClose, landmarks }) {
...
export default function Home() {
  ...
  const filteredSightings = sightings.filter((s) => currentLandmarkIds.includes(s.landmark_id))
    ...
    <div style={{ maxWidth: 480, margin: '0 auto', minHeight: '100vh', background: '#f5f9f7', fontFamily: "'Noto Sans KR','Pretendard',sans-serif", position: 'relative' }}>
      ...
      <div style={{ background: 'linear-gradient(180deg,#e8f4e8,#f5f9f7)', padding: '20px 20px 12px', position: 'relative', zIndex: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 24, fontWeight: 900, color: '#1b4332', display: 'flex', alignItems: 'center', gap: 8, letterSpacing: '-0.5px' }}>🦆 아기오리</h1>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
          {Object.values(STREAMS).map((s) => (
            <button key={s.id} onClick={() => setStream(s.id)} style={{
              flex: 1, padding: '10px 8px', borderRadius: 14, border: stream === s.id ? '2px solid #2d6a4f' : '1px solid rgba(76,145,115,.15)',
              background: stream === s.id ? 'linear-gradient(135deg,#e8f4e8,#d4ecf7)' : '#fff',
              fontSize: 14, fontWeight: stream === s.id ? 800 : 500, cursor: 'pointer',
              color: stream === s.id ? '#1b4332' : '#6b9080',
              fontFamily: "'Noto Sans KR',sans-serif", transition: 'all .2s',
              boxShadow: stream === s.id ? '0 2px 8px rgba(45,106,79,.1)' : 'none',
      ...
      <div style={{ padding: '16px 16px 100px', position: 'relative', zIndex: 10 }}>
        {loading ? (
          ...
          <>
            <StatsBar sightings={filteredSightings} />
            <SafetyTips />
            {tab === 'map' && <StreamMap sightings={filteredSightings} selected={selectedLandmark} onSelect={(lm) => { setSelectedLandmark(lm); setShowForm(true) }} landmarks={currentStream.landmarks} streamName={currentStream.name} />}
            {filteredSightings.length > 0 && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12, padding: '10px 14px', background: 'linear-gradient(135deg,#e8f8e0,#d4f1e0)', borderRadius: 12, border: '1px solid rgba(39,174,96,.15)' }}>
                ...
                <span style={{ fontSize: 13, color: '#1b4332', fontWeight: 600 }}>최근 목격: <strong>{ALL_LANDMARKS.find((l) => l.id === filteredSightings[0].landmark_id)?.name || filteredSightings[0].landmark_name}</strong> ({timeAgo(filteredSightings[0].spotted_at || filteredSightings[0].created_at)})</span>
              </div>
            )}
            {filteredSightings.length === 0 ? (
              <div style={{ textAlign: 'center', padding: 40, color: '#6b9080' }}>
                <div style={{ fontSize: 48, marginBottom: 8 }}>🐣</div>
                <div style={{ fontFamily: "'Noto Sans KR',sans-serif", fontSize: 15, fontWeight: 600 }}>{currentStream.name}에는 아직 제보가 없어요!</div>
